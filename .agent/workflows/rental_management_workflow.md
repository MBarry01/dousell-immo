...
| **Adresse** | `company_address` | **Adresse Légale** affichée sur le contrat. **Source unique** (Il n'y a PAS de champ adresse personnelle ailleurs). Remplir via "Configuration Premium" > "Adresse complète". |
| **Branding** | `logo_url`, `signature_url` | URLs publiques (Bucket `branding`). |

> [!IMPORTANT]
> Si le contrat affiche "Adresse non spécifiée", c'est que `company_address` est vide. L'utilisateur doit aller dans **Configuration Premium** et cliquer sur **Enregistrer** (l'upload de logo seul ne sauvegarde pas l'adresse).
