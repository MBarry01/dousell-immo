...
import { BUY_NOW_URL } from '@/components/saasable/utils/constant';
...
const linkProps: any = { target: '_blank', rel: 'noopener noreferrer' };
const menuItems = [
    {
        id: 'resources',
...
export default function Sitemap({ list, isMenuDesign }: any) {
            ...
            {(list || menuItems).map((item: any, index: number) => (
                <Grid key={index} {...item.grid}>
                    <Stack sx={{ alignItems: 'flex-start', gap: { md: 3 } }}>
                        ...
                        <MenuList>
                            {item?.menu &&
                                item?.menu.map((menu: any, i: number) => (
                                    <MenuItem
                                        key={i}
                                        disableRipple
