    ...
    return (
        <section className="p-6 md:p-8 bg-white dark:bg-gray-900/40 rounded-3xl border border-slate-200 dark:border-gray-800 space-y-8 shadow-sm dark:shadow-none">
                ...
                {isExpired ? (
                    ...
                    <div className="flex flex-col items-end gap-1">
                        <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-full px-4 py-1.5">
                            ...
                            <span className="text-amber-400 text-xs font-medium">{daysLeft} jours d'essai restants</span>
                        </div>
                        {trialEndsAt && (
            ...
            {isTrial && (
                <div className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-4">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Progression de l'essai gratuit</span>
            ...
            {isExpired && (stats.properties > 0 || stats.leases > 0) && (
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-6">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h3 className="text-white font-semibold mb-1 text-sm">Vos données sont préservées</h3>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                {/* STARTER */}
                <div className={`rounded-2xl border ${subscriptionTier === 'starter' ? 'border-[#F4C430] bg-[#F4C430]/5' : 'border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5'} p-6 flex flex-col`}>
                    <div className="flex items-center gap-2 mb-4">
                        <Rocket size={24} className="text-[#F4C430]" />
                    ...
                    <ul className="space-y-3 mb-8 flex-1">
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Jusqu'à 10 biens
                        </li>
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Documents standards
                        </li>
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Support standard
                        </li>
                    </ul>
                    <Button
                        onClick={() => handleUpgrade('starter')}
                        disabled={isSubmitting || (subscriptionTier === 'starter' && proStatus === 'active')}
                        className={`w-full h-10 text-sm ${subscriptionTier === 'starter' && proStatus === 'active' ? 'bg-zinc-800 text-white/50' : 'bg-[#F4C430] text-black hover:bg-[#F4C430]/90 font-bold'}`}
                    >
                ...
                {/* PRO */}
                <div className={`rounded-2xl border ${subscriptionTier === 'pro' ? 'border-[#F4C430] bg-[#F4C430]/5' : 'border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5'} p-6 flex flex-col relative`}>
                    <div className="absolute top-4 right-4 bg-[#F4C430] text-black text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                        Recommandé
                    </div>
                    <div className="flex items-center gap-2 mb-4">
                        <Crown size={24} className="text-[#F4C430]" />
                    ...
                    <ul className="space-y-3 mb-8 flex-1">
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Biens illimités
                        </li>
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Analyses financières
                        </li>
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Export de données
                        </li>
                        <li className="flex items-center gap-2 text-xs text-gray-600 dark:text-white/70">
                            <Check size={14} className="text-[#F4C430]" /> Support prioritaire
                        </li>
                    </ul>
                    <Button
                        onClick={() => handleUpgrade('pro')}
                        disabled={isSubmitting || (subscriptionTier === 'pro' && proStatus === 'active')}
                        className={`w-full h-10 text-sm ${subscriptionTier === 'pro' && proStatus === 'active' ? 'bg-zinc-800 text-white/50' : 'bg-[#F4C430] text-black hover:bg-[#F4C430]/90 font-bold'}`}
                    >
            ...
            <div className="bg-slate-900/50 dark:bg-black rounded-2xl p-6 text-center border border-slate-200 dark:border-white/5">
                ...
                <Button
                    size="sm"
                    className="h-9 px-6 border border-slate-300 dark:border-white/20 text-slate-700 dark:text-white bg-transparent hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-[#F4C430] transition-colors"
