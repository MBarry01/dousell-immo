        ...
        <div className="w-full mx-auto space-y-8 pb-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="relative overflow-hidden rounded-[2.5rem] bg-[#0F172A] shadow-2xl shadow-[#0F172A]/40 transition-all hover:shadow-[#0F172A]/50 active-press p-6 md:p-8">
                        {/* Status bar top — gold si dette, sinon discret */}
                        <div className={`absolute top-0 left-0 right-0 h-1 ${isUpToDate ? 'bg-white/10' : 'bg-[#F4C430]'}`} />
                        {/* Subtle radial glow */}
                        <div className="absolute -top-24 -right-24 w-72 h-72 bg-[#F4C430]/5 rounded-full blur-3xl pointer-events-none" />
                        ...
                        <div className="relative flex flex-col md:flex-row md:items-stretch justify-between gap-6 md:gap-8">
                            <div className="flex-1">
                                <header className="flex items-center gap-3 mb-6 pt-2">
                                    <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center font-black text-white text-lg shadow-lg ring-2 ring-white/10 transition-transform group-hover:scale-105">
                                        {tenantName?.[0]?.toUpperCase() || 'L'}
                                    </div>
                                    <div className="space-y-0">
                                        <p className="text-white/40 text-[9px] font-black uppercase tracking-[0.2em]">Espace Locataire</p>
                                        <h1 className="text-xl font-black text-white tracking-tighter leading-none">Bonjour, {tenantName?.split(' ')[0] || 'Locataire'}</h1>
                                    </div>
                                </header>

                                {/* Amount Display */}
                                <div className="bg-white/5 rounded-[2rem] p-6 border border-white/10 relative group/screen">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-baseline gap-2">
                                            <span className={`text-6xl font-black tracking-tighter ${isUpToDate ? 'text-white' : 'text-[#F4C430]'}`}>
                                                {formatCurrency(currentBalance)}
                                            </span>
                                            <span className="text-white/30 text-lg font-black italic">FCFA</span>
                                        </div>
                                    </div>

                                    <div className="mt-4 flex items-center justify-between">
                                        {isUpToDate ? (
                                            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/10 text-white/80 rounded-lg border border-white/10">
                                            ...
                                            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F4C430]/20 text-[#F4C430] rounded-lg border border-[#F4C430]/30">
                            ...
                            <div className="md:w-64 flex flex-col gap-3 self-end">
                                {!isUpToDate ? (
                                    <Button
                                        ...
                                        className="w-full h-14 bg-[#F4C430] hover:bg-amber-400 text-[#0F172A] font-black rounded-2xl transition-all shadow-xl shadow-[#F4C430]/30 hover:scale-[1.02] active:scale-95 text-sm group uppercase tracking-widest"
                                    >
                                        Payer maintenant
                                    ...
                                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-2">
                                        <div className="flex items-center gap-2 text-white/40">
                                            ...
                                            <span className="text-[9px] font-black uppercase tracking-widest">Prochaine échéance</span>
                                        </div>
                                        <p className="text-white font-black text-sm tracking-tight leading-none">{getNextRentInfo().split('-')[0]}</p>
                                        <p className="text-white/40 text-[10px] font-bold">{getNextRentInfo().split('-')[1]}</p>
                                    </div>
                                )}
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 shadow-sm ${isPaid
                                                    ? 'bg-[#0F172A]/5 group-hover:bg-[#0F172A]'
                                                    : 'bg-[#F4C430]/10 group-hover:bg-[#F4C430]'
                                                    }`}>
                                                    {isPaid
                                                        ? <CheckCircle2 className="w-5 h-5 transition-transform group-hover:rotate-12 text-[#0F172A] group-hover:text-[#F4C430]" />
                                                        : <Clock className="w-4 h-4 text-[#F4C430] group-hover:text-[#0F172A]" />
                                                    }
                                                </div>
                                                <div>
            ...
            {!isUpToDate && (
                <div className="fixed bottom-20 left-4 right-4 z-50 md:hidden animate-in slide-in-from-bottom-10">
                    <div className="bg-[#0F172A]/98 backdrop-blur-xl px-5 py-4 flex items-center justify-between rounded-3xl shadow-2xl border border-white/10">
                        <div>
                            <p className="text-[#F4C430] font-black text-lg tracking-tight">{formatCurrency(currentBalance)} <span className="text-[10px] font-bold text-white/30">FCFA</span></p>
                            <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">{pendingPayments.length} ÉCHÉANCE{pendingPayments.length > 1 ? 'S' : ''}</p>
                        </div>
                        <Button
                            onClick={() => setIsModalOpen(true)}
                            className="bg-[#F4C430] hover:bg-amber-400 text-[#0F172A] font-black rounded-2xl px-6 h-12 text-xs uppercase tracking-wider shadow-lg shadow-[#F4C430]/20 active:scale-95 transition-all"
                        >
                            Régler
                        </Button>
                    </div>
